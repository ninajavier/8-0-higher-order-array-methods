...
function findPinkElephantsByTimestreet(songs) {
  let object = songs.find(song => song.title === "Pink Elephants");
  return object;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let object = songs.find(song => song.runtimeInSeconds < 180);
  return object;
}
...
function findFirstTitleTrack(songs) {
  let object = songs.find(song => song.title === song.album);
  return object;
}
